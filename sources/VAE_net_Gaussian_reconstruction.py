...
def get_network_Gaussian_reconstruction(img_rows, img_cols, latent_dim):
#     img_rows = 400
#     img_cols = 400
...
# 10
    ...
    decoder_outputs = Conv2D(3, 1, activation='sigmoid', name='outputs_img')(deconvolution6)
    decoder = Model(inputs=[inputs_img, inputs_mask, decoder_input], outputs=decoder_outputs, name="decoder")
    decoder.summary()
    encoded_img1, encoded_img2, encoded_img3 = encoder([inputs_img, inputs_mask])
    decoded_img = decoder([inputs_img, inputs_mask,encoded_img3])
    autoencoder = Model([inputs_img, inputs_mask], decoded_img, name="autoencoder")
    autoencoder.summary()
